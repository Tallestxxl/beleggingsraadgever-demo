...
class AdvisorTests(unittest.TestCase):
    ...
    def test_cash_shortfall_lists_sell_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = SQLiteRepository(Path(tmp) / "test.sqlite")
            repo.init()
            repo.save_investor_profile(
            ...
            repo.upsert_portfolio_asset(
                PortfolioAsset(asset_type="cash", value=20000, currency="EUR", as_of="2026-05-05")
            )
            for symbol, price in [("WEAK", 50), ("QUALITY", 50)]:
                repo.upsert_portfolio_position(
                    PortfolioPosition(
                        symbol=symbol,
                        quantity=1000,
                ...
                repo.upsert_portfolio_price(
                    PortfolioPrice(symbol=symbol, as_of="2026-05-05", close_price=price, currency="EUR")
                )
            repo.upsert_financial_snapshot(
                FinancialSnapshot(
                    symbol="WEAK",
            ...
            self.assertEqual(report.portfolio_fit.cash_shortfall, 5000)
            self.assertEqual(report.portfolio_fit.practical_buy_amount, 0)
            self.assertEqual(report.portfolio_fit.sell_candidates[0].symbol, "WEAK")
            self.assertEqual(report.portfolio_fit.sell_candidates[0].suggested_sale_value, 5000)
            self.assertIn("Verkoopkandidaat WEAK", markdown)
